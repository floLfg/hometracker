<template>
    <div>
        <form v-on:submit.prevent="addAmount()" class="participation-create">
            <legend class="smaller">Ajouter un montant</legend>
            <input type="number" ref="amount" name="amount" v-model="amount" class="form-control" step="0.01">
        ...
        methods: {
            addAmount() {
                axios.post('api/savings/'+this.saving.id+'/participations?amount='+this.amount).then((response) => {
                    ...
                    this.$root.$refs.toastr.s('Montant ajouté');
                    this.amount = 0;
